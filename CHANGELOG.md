...
- Причина: full-replace был неудобным и опасным паттерном для UI, где владелец правит один день за раз — риск случайно снести остальные дни при неполном payload. Отмечено как TODO в предыдущей сессии, доведено до конца по просьбе пользователя
- `WorkingHoursServiceTest` переписан под `upsertDay` (создание нового дня, обновление существующего на месте, owner-check, дефолт `isDayOff=false`, валидация open/close, overnight-расписание); один тест стал дублем после удаления параметра и убран, как в аналогичном фиксе `BookingServiceTest` ранее в этом файле
- Полный прогон бэкенд-тестов: 241/243 (2 падения — известное ограничение песочницы на Testcontainers, см. память dev-окружения, не регрессия)
- Заодно замечено (не исправлено, вне скоупа этой правки): любой неподдерживаемый HTTP-метод на существующем пути (например, PUT там, где есть только GET/PATCH) отдаёт `500 Internal Server Error` вместо `405 Method Not Allowed` — `GlobalExceptionHandler` не имеет отдельного хендлера для `HttpRequestMethodNotSupportedException` и заворачивает её в generic `GEN_002`. Подтверждено, что это уже существовало на других, не тронутых этой правкой эндпоинтах (`PUT /api/v1/halls`) — не регрессия конкретно этого фикса
- Проверено вживую через curl: `PATCH .../working-hours/3` возвращает `200` и обновляет только день 3; старый `PUT .../working-hours` больше не существует как ожидаемый маршрут

### Fixed — владельцы ресторанов не могли получить доступ к собственной аналитике и управлению бронями (роль, которая нигде не выдаётся)
- `AnalyticsController` был закрыт классовым `@PreAuthorize("hasAnyRole('RESTAURANT_ADMIN', 'COMPANY_ADMIN', 'SUPER_ADMIN')")`, `BookingController.getRestaurantBookings` — тем же на уровне метода. Но ни один код-путь в проекте никогда не выдаёт пользователю `ROLE_RESTAURANT_ADMIN`/`ROLE_COMPANY_ADMIN` — `AuthService.register()` всегда ставит только `ROLE_USER`, а `CompanyService.create()`/`RestaurantService.create()` роль не трогают. Обычный владелец, зарегистрировавшийся → создавший компанию → создавший ресторан (стандартный онбординг, которым пользуется вся веб-админка), навсегда остаётся с `ROLE_USER` и получал `403 Access Denied` на **собственный** ресторан, несмотря на корректный owner-check в сервисном слое (`AnalyticsService.getRestaurantAnalytics`, `BookingService.getRestaurantBookings`)
- То же самое, но хуже, было в `BookingService.updateStatus()`: `isManager` вычислялся в контроллере из тех же никогда не выдаваемых ролей и комбинировался с owner-check (`managesThisRestaurant = isManager && ...owner...id().equals(userId)`) — это был правильный фикс IDOR из более ранней записи этого файла (см. ниже), но раз `isManager` был всегда `false` для реального владельца, комбинация тоже была всегда `false`. Итог: владелец не мог подтвердить/отклонить/завершить **ни одну** бронь в своём же ресторане, только отменить свою же бронь как обычный клиент
- Найдено при подготовке дашборда аналитики в `keepbooking-admin` — тестовый аккаунт получал `Access Denied` на пустом месте; расследование показало, что это системная проблема (роль нигде не выдаётся), а не баг одного эндпоинта, и что она же тихо ломала уже собранную страницу управления бронями
- Фикс: убрал ролевую проверку из `AnalyticsController` и `BookingController.getRestaurantBookings` (теперь `isAuthenticated()`, как во всех остальных owner-scoped контроллерах — `HallController`, `MenuItemController`, `WorkingHoursController`, `RestaurantController`); `BookingService.updateStatus()` больше не принимает `isManager` вообще — «менеджер» это просто владелец ресторана этой брони (`booking.getRestaurant().getCompany().getOwner().getId().equals(userId)`), без каких-либо ролей. `ROLE_RESTAURANT_ADMIN`/`ROLE_COMPANY_ADMIN`/`ROLE_SUPER_ADMIN` в энаме остаются (возможно, задел под будущую админку с назначением ролей), но ни на один из этих трёх эндпоинтов больше не влияют
- `BookingServiceTest` обновлён под новую сигнатуру `updateStatus(bookingId, userId, request)` без `isManager`; убран дублирующийся regression-тест, ставший неотличим от `updateStatusThrowsAccessDeniedWhenActorIsNeitherOwnerNorManager` после удаления флага. Полный прогон бэкенд-тестов: 241/243 (2 падения — известное ограничение песочницы на Testcontainers, не регрессия, см. память dev-окружения)
- Проверено вживую через curl с тремя реальными аккаунтами: владелец ресторана получает `200` на `GET /restaurants/{id}/analytics`, `GET /bookings/restaurant/{id}` и `PATCH /bookings/{id}/status` (подтвердил реальную бронь PENDING→CONFIRMED); посторонний зарегистрированный пользователь по-прежнему получает `403` на всех трёх — IDOR-защита из более ранней записи не пострадала, просто теперь основана только на владении, а не на владении+недостижимой роли
