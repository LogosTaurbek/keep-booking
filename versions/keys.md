...
- Итого: **145 unit-тестов реально прогнаны и зелёные** (подтверждено по XML-отчётам) + 1 integration-тест подтверждён в реальном CI.
- Не покрыто автотестами: `AdminStatsService` (чистая агрегация `count()`, низкая ценность юнит-теста) — тестировалось только вручную через curl. Практически все модули с содержательной бизнес-логикой теперь закрыты unit-тестами.
