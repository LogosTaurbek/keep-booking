...
- `NotificationServiceTest` (unit/Mockito, 5 тестов) — markAsRead (не найдено/чужое → `ACCESS_DENIED`/успех), markAllAsRead делегирует в bulk-update репозитория.
- `SearchHistoryServiceTest` (unit/Mockito, 6 тестов) — `record()` тихо не сохраняет запись, если userId == null или нет ни одного фильтра (анонимный поиск / пустой поиск не засоряют историю); сохраняет при наличии хотя бы одного фильтра (name/cityId/minRating).
- `RateLimitServiceTest` (unit/Mockito, 4 теста) — контракт сравнения с лимитом (`tryConsume` true при current <= limit, false при превышении, fail-closed при null от Redis). Сама атомарность INCR+PEXPIRE — свойство Lua-скрипта, юнит-тестом не проверяется.
- `RateLimitFilterTest` (unit/Mockito, 7 тестов, `MockHttpServletRequest/Response`) — обход при `enabled=false` и на exempt-путях (`/actuator/**` и т.д.), выбор auth- vs general-лимита/окна по префиксу `/api/v1/auth/`, приоритет `X-Forwarded-For` над `remoteAddr` при определении ключа клиента, форма 429-ответа (статус, `Retry-After`, тело с кодом ошибки).
- `RestaurantSpecificationsTest` (unit/Mockito, 9 тестов, мокнутые `Root`/`CriteriaBuilder`/`CriteriaQuery`) — каждый опциональный фильтр возвращает `null` Predicate при null/blank входе (критично: Spring Data отбрасывает null-предикаты при AND-комбинировании через `Specification.where`, а не-null "поймать всё/ничего" предикат тихо сломал бы поиск), корректный predicate/join/distinct при непустом значении.
- `RestaurantServiceTest` (unit/Mockito, 9 тестов) — create (не своя компания/дефолт timezone=UTC), getById, getMyRestaurants (агрегация по всем компаниям владельца), getByStatus, moderate (approve/block).
- `UserServiceTest` (unit/Mockito, 9 тестов, реальный `UserMapperImpl`) — getProfile (не найден/soft-deleted → `USER_NOT_FOUND`), updateProfile (частичный update, город не найден, город найден), deleteAccount (анонимизация email + `DELETED`), setBlocked.
- `WorkingHoursServiceTest` (unit/Mockito, 6 тестов) — owner-check, `replaceWeek` полностью удаляет старые записи перед вставкой новых (delete-then-insert, не upsert), `isDayOff` дефолтится в `false`, когда `null` в запросе.
- `BookingSchedulerServiceTest` (unit/Mockito, реальный `SimpleMeterRegistry`, 4 теста) — `autoCancelExpiredPending`/`autoCompletePastBookings`: no-op на пустой список, иначе меняют статус + шлют уведомление + пишут audit log + инкрементят метрику на каждую бронь.
- `IdempotencyServiceTest` (unit/Mockito, 5 тестов) — **нашёл и исправил реальный прод-баг**: `BookingDto` (`@Data @Builder`, без `@NoArgsConstructor`) — единственный DTO в кодовой базе, который Jackson реально десериализует (`IdempotencyService.get()` читает его обратно из Redis), и без no-args конструктора десериализация ВСЕГДА падала на `InvalidDefinitionException`, тихо перехватывалась и трактовалась как cache miss — Redis-кэш идемпотентности не давал вообще никакого эффекта с самого начала (маскировалось DB-фоллбэком по `idempotency_key`, поэтому curl-тестированием это не поймать). Исправлено добавлением `@NoArgsConstructor @AllArgsConstructor` на `BookingDto`.
- Итого: **178 unit-тестов реально прогнаны и зелёные** (подтверждено по XML-отчётам) + 1 integration-тест подтверждён в реальном CI.
- Не покрыто автотестами: `AdminStatsService` (чистая агрегация `count()`, низкая ценность юнит-теста) — тестировалось только вручную через curl. Все модули с содержательной бизнес-логикой теперь закрыты unit-тестами.
...
- [x] Трейсинг (OpenTelemetry) — `micrometer-tracing-bridge-otel` + `opentelemetry-exporter-otlp`, экспорт по OTLP/HTTP на `management.otlp.tracing.endpoint` (по умолчанию `http://localhost:4318/v1/traces`), sampling 100% (`management.tracing.sampling.probability`, оба env-переопределяемые). В `docker-compose.yml` добавлен `jaeger` (all-in-one, порт 16686 — UI, 4318 — OTLP receiver) для локального просмотра трейсов. Web-запросы и Spring Security filter chain трассируются автоматически (авто-инструментация из коробки), `@Scheduled`-задачи `BookingSchedulerService` — тоже (проверено вживую: видно как отдельные трейсы `booking-scheduler-service.auto-cancel-expired`). traceId/spanId кладутся в MDC micrometer-tracing и попадают и в JSON-логи (добавлены в `includeMdcKeyName` LogstashEncoder), и в консоль local-профиля (`[requestId,traceId,spanId]` в паттерне) — сквозная корреляция логов и трейсов. Проверено вживую через docker-compose + bootRun + curl + Jaeger API (`/api/services`, `/api/traces`).
